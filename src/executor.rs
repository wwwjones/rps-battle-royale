...
use crate::planner::brain::{BRAINConfiguration, Brain};
...
{
    /// The attached brain configuration
    ...
    threads: HashMap<AgentId, JoinHandle<Brain<D>>>,
...
{
    /// Creates a new executor, initializes state and task queue from the S trait.
    pub fn new(brain_config: BRAINConfiguration, executor_state: &'a mut S) -> Self {
        let state = executor_state.create_initial_state();
        let task_queue = executor_state.init_task_queue(&state);
    ...
    fn new_brain(&self, start_tick: u64, root_agent: AgentId) -> Brain<D> {

        Brain::new(root_agent, D::derive_local_state(&self.state, root_agent), start_tick, self.brain_config.depth)
...
/*         let planning_task_duration = self
        ...
        let early_stop_condition: Option<Box<EarlyStopCondition>> = Some(Box::new(move |visits| {
            visits >= S::MINIMUM_VISITS
                && tick_atomic.load(Ordering::Relaxed) >= start_tick + planning_task_duration.get() - 1
        }));
        MCTS::<D>::new_with_tasks(
            D::derive_local_state(&self.state, root_agent),
            root_agent,
            start_tick,
            self.queue.task_queue.clone(),
    ...
    fn block_on_planning(&mut self, tick: u64) {
            ...
            .filter(|task| task.end <= tick && task.task.downcast_ref::<PlanningTask>().is_some())
        {
            ...
            let best_task = brain.best_task().unwrap_or_else(|| Box::new(IdleTask));
        ...
        for active_task in active_tasks.iter().filter(|task| task.end <= tick) {
            ...
            let new_task = new_task.unwrap_or_else(|| {
                Box::new(PlanningTask(
                    self.brain_config.planning_task_duration.unwrap(),
                ))
            });
            ...
            if new_task.downcast_ref::<PlanningTask>().is_some() {
                let mut brain = self.new_brain(tick, active_agent);
                if log::log_enabled!(log::Level::Info) {
                    log::info!(
                ...
                let handle = thread::Builder::new()
                    ...
                    .spawn(move || {
                        // Initialize brain instance for planning
...
{
    // Initialize the state and the agent queue
    let mut executor = ThreadedExecutor::<D, S>::new(brain_config.clone(), executor_state);
    loop {
        if !executor.step() {
            break;
        }
...
/*         let mcts_config = MCTSConfiguration {
        ...
        let mut executor = ThreadedExecutor::new(brain_config, &mut executor_state);
